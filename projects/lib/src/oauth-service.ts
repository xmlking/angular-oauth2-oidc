...
export class OAuthService extends AuthConfig {
  ...
  private fetchToken(params: HttpParams): Promise<object> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.http.post<TokenResponse>(this.tokenEndpoint, params, { headers }).subscribe(
        ...
        err => {
          this.logger.error('Error getting token', err);
  ...
  protected async createLoginUrl(
    ...
    if (!this.disableNonceCheck) {
      nonce = await this.createAndSaveNonce();
      if (state) {
    ...
    if (this.pkceFlow) {
      url += '&code_challenge=' + this.generateCodeChallenge();
  ...
  protected initAuthorizationCodeFlowInternal(): void {
    if (!this.validateUrlForHttps(this.loginUrl)) {
      throw new Error('loginUrl must use Http. Also check property requireHttps.');
    }

    this.createLoginUrl(this.createAndSaveState(), '', null, false, {})
      .then(function(url) {
        location.href = url;
      })
      .catch(error => {
  ...
  public tryLogin(options: LoginOptions = null): Promise<boolean> {
    ...
    if (
      ...
      (window.location.search.startsWith('?code=') || window.location.search.includes('&code='))
    ) {
  ...
  public tryLoginImplicit(options: LoginOptions = null): Promise<boolean> {
    ...
    if (this.requestAccessToken && !options.disableOAuth2StateCheck) {
      const success = this.validateNonceForAccessToken(accessToken, nonceInState);
      if (!success) {
   ...
   * @ignore
   */
  public async processIdToken(idToken: string, accessToken: string): Promise<ParsedIdToken> {
    ...
    if (!this.disableAtHashCheck && this.requestAccessToken && !(await this.checkAtHash(validationParams))) {
